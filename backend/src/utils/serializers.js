...
export function serializeProject(project, currentUserId = null) {
  ...
  return {
    ...
    participants_needed: project.participantsNeeded ?? project.capacity,
    minimum_xp_required: project.minimumXpRequired ?? 0,
    xp_commitment_stake: project.xpCommitmentStake ?? 0,
    ...
    daily_reporting_required: Boolean(project.dailyReportingRequired),
    minimum_contribution_score: project.minimumContributionScore ?? 70,
    review_frequency: project.reviewFrequency || "Daily",
    mentor_review_required: Boolean(project.mentorReviewRequired),
    reward_pool_xp: project.rewardPoolXp ?? 0,
    stake_refund_policy: project.stakeRefundPolicy || "enabled",
    performance_multiplier: project.performanceMultiplier ?? 1,
    dropout_penalty: project.dropoutPenalty ?? 0,
    inactive_penalty: project.inactivePenalty ?? 0,
...
export function serializeApplication(application) {
  ...
  return {
    ...
    project_role: application.projectRole || "Contributor",
    committed_xp: application.committedXp || 0,
    commitment_status: application.commitmentStatus || "none",
    coordinator: Boolean(application.coordinator),
    contribution_score: application.contributionScore || null,
    reward_eligible: Boolean(application.rewardEligible),
    reward_xp: application.rewardXp || 0,
    settlement_status: application.settlementStatus || "pending",
