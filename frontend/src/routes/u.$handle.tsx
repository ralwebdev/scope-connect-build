...
function PublicProfilePage() {
  ...
  const avatar = useImageSrc(u?.avatarUrl);
  if (!u) return null;
    ...
    <AppShell>
      <section className="border-b border-border/40 bg-gradient-hero py-12 text-primary-foreground">
        <div className="mx-auto flex max-w-4xl flex-wrap items-center gap-6 px-4 sm:px-6 lg:px-8">
          <div className="flex h-24 w-24 items-center justify-center overflow-hidden rounded-2xl text-3xl font-bold text-brand-foreground shadow-brand ring-2 ring-white/10"
            style={{ background: avatar.hasImage ? "transparent" : u.avatarColor }}>
            {avatar.hasImage ? (
              <img src={avatar.src} alt={u.name} className="h-full w-full object-cover" onError={avatar.onError} />
            ) : (
