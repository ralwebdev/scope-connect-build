...
function SupportPage() {
  ...
  // Restore last opened category from sessionStorage so navigating away &
  // back keeps context. Recent searches persist in localStorage.
  useEffect(() => {
    try {
  ...
  useEffect(() => {
    try { sessionStorage.setItem(LAST_CAT_KEY, activeCat); } catch { /* noop */ }
  }, [activeCat]);
  ...
  // Auto-open first match when search yields results
  useEffect(() => {
    if (isSearching && matches.length > 0) {
      setOpenKey(matches[0].q);
    }
  }, [isSearching, matches]);
  ...
  const grouped = useMemo(() => {
    ...
    FAQS.forEach((f) => map[f.cat].push(f));
    return map;
  }, []);
  ...
  const persistRecent = (term: string) => {
    if (!term.trim()) return;
    try {
      const next = [term.trim(), ...recent.filter((r) => r.toLowerCase() !== term.toLowerCase())].slice(0, 5);
      setRecent(next);
          ...
          <Card className="mt-4 p-8 text-center">
            ...
            <div className="mt-4 flex flex-wrap justify-center gap-2">
              {CATEGORIES.slice(0, 4).map((c) => (
                <Button key={c} size="sm" variant="outline" onClick={() => { setQuery(""); setActiveCat(c); }}>
                  {c}
                </Button>
              ))}
            </div>
